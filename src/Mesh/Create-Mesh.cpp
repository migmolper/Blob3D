...
static PetscErrorCode CreateMesh_DMDA(DM* FE_Mesh,                //!
  ...
  if (local_dx < 2 * r_cutoff_V) {
    PetscCall(PetscError(PETSC_COMM_WORLD, __LINE__, "CreateMesh_DMDA",
                         __FILE__, PETSC_ERR_RETURN, PETSC_ERROR_INITIAL,
...
static PetscErrorCode CreateMesh_DMPLEX(DM* FE_Mesh,                //!
  ...
  const PetscReal box_ll[3] = {Simulation_file.box_x_min,
                               Simulation_file.box_y_min,
                               Simulation_file.box_z_min};
  const PetscReal box_ur[3] = {Simulation_file.box_x_max,
                               Simulation_file.box_y_max,
                               Simulation_file.box_z_max};

  PetscCall(DMPlexCreateBoxMesh(PETSC_COMM_WORLD, dim, Is_simplex, faces,
                                box_ll, box_ur, periodicity, interpolate,
                                overlap, sparseLocalize, FE_Mesh));
  PetscCall(PetscObjectSetName((PetscObject)*FE_Mesh, "plex-DMD"));

  PetscCall(DMLocalizeCoordinates(*FE_Mesh));

  /* Distribute mesh over processes */
  DM FE_Mesh_distributed;
  PetscCall(DMPlexDistribute(*FE_Mesh, 1, NULL, &FE_Mesh_distributed));
  if (FE_Mesh_distributed) {
    PetscCall(DMDestroy(FE_Mesh));
    *FE_Mesh = FE_Mesh_distributed;
  }
  PetscCall(DMSetFromOptions(*FE_Mesh));
  {
    PetscInt numComp[] = {1};
    PetscInt numDof[] = {1, 0, 0};
    PetscInt numBC = 0;
    PetscSection section;

    PetscCall(DMPlexCreateSection(*FE_Mesh, NULL, numComp, numDof, numBC, NULL,
                                  NULL, NULL, NULL, &section));
    PetscCall(DMSetLocalSection(*FE_Mesh, section));
...
PetscErrorCode CreateMesh(DM* FE_Mesh, dump_file Simulation_file,
  ...
  switch (type) {
    ...
    default:
      PetscCall(PetscError(PETSC_COMM_WORLD, __LINE__, "CreateMesh(_,_,*,_)",
                           __FILE__, PETSC_ERR_RETURN, PETSC_ERROR_INITIAL,
                           "Incorrect Box_Type option: %i", type));
      PetscFunctionReturn(PETSC_ERR_RETURN);
      break;
  }
